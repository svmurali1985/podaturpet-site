...
pattern = re.compile(
    r'(<div class="hero-inner">\s*)'
    r'<div class="eyebrow">.*?</div>\s*'
    r'<h1>.*?</h1>\s*'
    r'<p class="hero-copy">.*?</p>\s*'
    r'<div class="buttons">.*?</div>',
    re.S
)

replacement = '''\\1<div class="eyebrow">Podaturpet • Tamil Nadu • India • Woven Trust</div>
...
if count != 1:
    raise SystemExit(f"ERROR: expected to update 1 hero section, updated {count}. No file was written.")
